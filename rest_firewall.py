...
headers = [('Access-Control-Allow-Origin','*'), ('Access-Control-Allow-Methods','GET, POST, PUT, DELETE, OPTIONS'), ('Access-Control-Allow-Headers','X-Accept, X-Accept-Charset, Content-Type')]
...
class RestFirewallAPI(app_manager.RyuApp):
	...
	mapper.connect('firewall', uri,
		       controller=FirewallController, action='response_ok',
...
class FirewallController(ControllerBase):
    ...
    def _access_module(self, switchid, func, waiters=None):
        ...
        return Response(content_type='application/json', body=body, headerlist=headers)
    ...
    # OPTIONS /firewall/rules/{switchid}
    def response_ok(self, req, switchid, **_kwargs):
	return Response(status=200, headerlist=headers)
    ...
    def _get_rules(self, switchid, vlan_id=VLANID_NONE):
        try:
            ...
            return Response(status=400, body=str(message), headerlist=headers)
